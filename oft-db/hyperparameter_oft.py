...
def main(args):
    ...
    if args.with_prior_preservation:
        ...
        if cur_class_images < args.num_class_images:
            ...
            images = [
                pipeline(args.class_prompt, num_inference_steps=25, generator=generator).images[0]
                for _ in tqdm(range(num_new_images))
            ]
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers("hyperparamter-cot", config=vars(args), init_kwargs=wandb_init)
    ...
    inputs = tokenizer([args.class_prompt], padding=True, return_tensors="pt")
    text_features = model.get_text_features(**inputs)
    # print('text feature', text_features.shape) # output shape of text features [1, 768]

    similarity = []
    for filename in tqdm(os.listdir(args.class_data_dir)):
        if filename.endswith('.jpg') or filename.endswith('.jpeg') or filename.endswith('.png'):
            # Load the image using PIL
            image_path = os.path.join(args.class_data_dir, filename)
