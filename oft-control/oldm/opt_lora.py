...
class OPTInjectedLinear(nn.Module):
    def __init__(
...
class OPTInjectedLinear_with_norm(nn.Module):
    def __init__(
        self, in_features, out_features, bias=False, r=4, dropout_p=0.1, scale=1.0, dim=3
    ):
        super().__init__()
...
class OPTInjectedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, bias=True, dim=3):
        ...
        self.OPT = nn.Conv2d(self.in_channels, self.out_channels, self.kernel_size, stride=self.stride, padding=self.padding, bias=self.bias)
...
def _find_modules_v2(
    ...
    results = []
    # For each target find every linear_class module that isn't a child of a OPTInjectedLinear
        ...
        for fullname, module in ancestor.named_modules():
            if any([isinstance(module, _class) for _class in search_class]):
                ...
                *path, name = fullname.split(".")
                ...
                # Skip this linear if it's a child of a OPTInjectedLinear
...
def _find_modules_old(
    ...
    search_class: List[Type[nn.Module]] = [nn.Linear],
    exclude_children_of: Optional[List[Type[nn.Module]]] = [OPTInjectedLinear],
):
    ret = []
...
def inject_trainable_opt(
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if verbose:
            print("OPT Injection : injecting lora into ", name)
            print("OPT Injection : weight shape", weight.shape)
        _tmp = OPTInjectedLinear(
...
def inject_trainable_opt_with_norm(
    model: nn.Module,
    target_replace_module: Set[str] = DEFAULT_TARGET_REPLACE,
    loras=None,  # path to lora .pt
    verbose: bool = False,
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if verbose:
            print("OPT Injection : injecting lora into ", name)
            print("OPT Injection : weight shape", weight.shape)
        _tmp = OPTInjectedLinear_with_norm(
            _child_module.in_features,
            _child_module.out_features,
            _child_module.bias is not None,
            dropout_p=dropout_p,
...
def inject_trainable_opt_extended(
    model: nn.Module,
...
def extract_lora_ups_down(model, target_replace_module=DEFAULT_TARGET_REPLACE):
    ...
    for _m, _n, _child_module in _find_modules(
        ...
        search_class=[OPTInjectedLinear, OPTInjectedConv2d],
    ):
...
def collapse_lora(model, alpha=1.0):
    ...
    for _module, name, _child_module in _find_modules(
        ...
        if isinstance(_child_module, OPTInjectedLinear):
...
def monkeypatch_or_replace_lora(
    ...
    for _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class=[nn.Linear, OPTInjectedLinear]
    ):
        _source = (
...
def monkeypatch_or_replace_lora_extended(
    ...
    for _module, name, _child_module in _find_modules(
        model,
        target_replace_module,
        search_class=[nn.Linear, OPTInjectedLinear, nn.Conv2d, OPTInjectedConv2d],
    ):

        if (_child_module.__class__ == nn.Linear) or (
            _child_module.__class__ == OPTInjectedLinear
        ):
            ...
            _child_module.__class__ == OPTInjectedConv2d
        ):
            if len(loras[0].shape) != 4:
                continue
            _source = (
                _child_module.conv
                if isinstance(_child_module, OPTInjectedConv2d)
                else _child_module
            )
...
def monkeypatch_remove_lora(model):
    for _module, name, _child_module in _find_modules(
        model, search_class=[OPTInjectedLinear, OPTInjectedConv2d]
    ):
        if isinstance(_child_module, OPTInjectedLinear):
...
def monkeypatch_add_lora(
    ...
    for _module, name, _child_module in _find_modules(
        model, target_replace_module, search_class=[OPTInjectedLinear]
    ):
...
def tune_lora_scale(model, alpha: float = 1.0):
    for _module in model.modules():
        if _module.__class__.__name__ in ["OPTInjectedLinear", "OPTInjectedConv2d"]:
            _module.scale = alpha
