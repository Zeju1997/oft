...
from cldm.opt import ResBlock_opt, OPT_cp, TimestepBlock
...
class ControlNet(nn.Module):
    ...
    def forward(self, x, hint, timesteps, context, **kwargs): # hint: torch.Size([4, 3, 512, 512])
        outs = []
        t_emb = timestep_embedding(timesteps, self.model_channels, repeat_only=False)
        emb = self.time_embed(t_emb)
...
class ControlLDM(LatentDiffusion):
    ...
    # only update the R of OPT layers
    def configure_optimizers(self):
        lr = self.learning_rate

        params = []
        names = []
        # Disable requires_grad to save computation time        
        for name, param in self.control_model.named_parameters():
            if 'hint' in name or '.R' in name:
                params.append(param)
                print("Parameter added:", name)
                names.append(name)
                print("---")
