...
class OFTInjectedLinear_vanilla(nn.Module):
    def __init__(
...
class OFTInjectedLinear(nn.Module):
    def __init__(
        self, in_features, out_features, bias=False, r=4, eps=1e-5, is_coft=True, block_share=False, n_butterfly_block=5
    ):
        super().__init__()
        ...
        # Define the reduction rate:
        self.r = in_features / 2
        self.is_coft = is_coft

        n_butterfly_block = int(torch.log2(in_features))
        for i in range(n_butterfly_block):
            self.register_buffer(f'P_{i}', torch.rand(3,3))
